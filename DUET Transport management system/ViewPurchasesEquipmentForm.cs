...
{
    public partial class ViewPurchasesEquipmentForm : Form
    {
        public ViewPurchasesEquipmentForm()
        {
            InitializeComponent();
        }

        private void ViewPurchasesEquipmentForm_Load(object sender, EventArgs e)
        ...
        private void CreatePdf_Click(object sender, EventArgs e)
        {
            ...
            PdfPTable pdfTable = new PdfPTable(ViewPurchaseGrid.ColumnCount);
            PdfPCell cell = new PdfPCell(new Phrase(" Purchase Equipment List", new iTextSharp.text.Font(iTextSharp.text.Font.NORMAL, 25f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLUE)));
            cell.BackgroundColor = new BaseColor(0, 150, 0);
            cell.Colspan = ViewPurchaseGrid.ColumnCount;
            ...
            for (int j = 0; j < ViewPurchaseGrid.Columns.Count; j++)
            {
                pdfTable.AddCell(new Phrase(ViewPurchaseGrid.Columns[j].HeaderText));
            }
            pdfTable.HeaderRows = 1;

            for (int i = 0; i < ViewPurchaseGrid.Rows.Count; i++)
            {
                for (int k = 0; k < ViewPurchaseGrid.Columns.Count; k++)
                {
                    if (ViewPurchaseGrid[k, i].Value != null)
                        pdfTable.AddCell(new Phrase(ViewPurchaseGrid[k, i].Value.ToString()));
                }
            }
            ...
            using (FileStream stream = new FileStream(folderPath + "purchase equipment list.pdf", FileMode.Create))
            {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select * from purchase_equipments where date>='" + d1 + "'and date<='" + dateTimePicker2.Value + "';", cnn);
            //cnn.Open();
            try
            {
                ...
                ViewPurchaseGrid.DataSource = bsource;
                sda.Update(dt);
            }
            catch (Exception ex)
            {
        ...
        private void CreatePdf_Click_1(object sender, EventArgs e)
        {
            //Creating iTextSharp Table from the DataTable data
            PdfPTable pdfTable = new PdfPTable(ViewPurchaseGrid.ColumnCount);
            PdfPCell cell = new PdfPCell(new Phrase(" Purchase Equipment List", new iTextSharp.text.Font(iTextSharp.text.Font.NORMAL, 25f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLUE)));
