...
{
    public partial class ViewUsageEquipment : Form
    {
        public ViewUsageEquipment()
        {
            InitializeComponent();
        }

        private void ViewUsageEquipment_Load(object sender, EventArgs e)
        {
        ...
        private void CreatePdf_Click(object sender, EventArgs e)
        {
            ...
            PdfPTable pdfTable = new PdfPTable(ViewUsagesEquipmentGrid.ColumnCount);
            PdfPCell cell = new PdfPCell(new Phrase(" Usage Equipment List", new iTextSharp.text.Font(iTextSharp.text.Font.NORMAL, 25f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLUE)));
            cell.BackgroundColor = new BaseColor(0, 150, 0);
            cell.Colspan = ViewUsagesEquipmentGrid.ColumnCount;
            ...
            for (int j = 0; j < ViewUsagesEquipmentGrid.Columns.Count; j++)
            {
                pdfTable.AddCell(new Phrase(ViewUsagesEquipmentGrid.Columns[j].HeaderText));
            }
            pdfTable.HeaderRows = 2;

            for (int i = 0; i < ViewUsagesEquipmentGrid.Rows.Count; i++)
            {
                for (int k = 0; k < ViewUsagesEquipmentGrid.Columns.Count; k++)
                {
                    if (ViewUsagesEquipmentGrid[k, i].Value != null)
                        pdfTable.AddCell(new Phrase(ViewUsagesEquipmentGrid[k, i].Value.ToString()));
                }
            }
            ...
            using (FileStream stream = new FileStream(folderPath + "Usage equipmentlist.pdf", FileMode.Create))
            {
        ...
        private void ViewUsagesEquipmentGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void perchaseEquipmentToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select * from Usage_equipment where date>='" + dateTimePicker1.Value + "'and date<='" + dateTimePicker2.Value + "';", cnn);
            //cnn.Open();
            try
            {
                ...
                ViewUsagesEquipmentGrid.DataSource = bsource;
                sda.Update(dt);
            }
            catch (Exception ex)
            {
